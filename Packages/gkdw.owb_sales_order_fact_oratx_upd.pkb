DROP PACKAGE BODY GKDW.OWB_SALES_ORDER_FACT_ORATX_UPD;

CREATE OR REPLACE PACKAGE BODY GKDW."OWB_SALES_ORDER_FACT_ORATX_UPD" AS
...
CURSOR "FLTR_c" IS
  SELECT
  "SALES_ORDER_FACT"."SALES_ORDER_ID" "SALES_ORDER_ID",
  "ORACLETX_HISTORY"."CREATEDATE" "CREATEDATE"
FROM
    "SALES_ORDER_FACT"  "SALES_ORDER_FACT"   
 LEFT OUTER JOIN   "SLXDW"."ORACLETX_HISTORY"  "ORACLETX_HISTORY" ON ( ( ( "ORACLETX_HISTORY"."EVXEVENTID" = "SALES_ORDER_FACT"."SALES_ORDER_ID" ) ) AND ( ( "ORACLETX_HISTORY"."TRANSACTIONTYPE" = "OWB_SALES_ORDER_FACT_ORATX_UPD"."GET_CONST_0_TRANSACTIONTYPE" ) ) )
  WHERE 
...
CURSOR "FLTR_c$1" IS
  SELECT
  "SALES_ORDER_FACT"."SALES_ORDER_ID" "SALES_ORDER_ID$1",
  "ORACLETX_HISTORY"."CREATEDATE" "CREATEDATE"
FROM
    "SALES_ORDER_FACT"  "SALES_ORDER_FACT"   
 LEFT OUTER JOIN   "SLXDW"."ORACLETX_HISTORY"  "ORACLETX_HISTORY" ON ( ( ( "ORACLETX_HISTORY"."EVXEVENTID" = "SALES_ORDER_FACT"."SALES_ORDER_ID" ) ) AND ( ( "ORACLETX_HISTORY"."TRANSACTIONTYPE" = "OWB_SALES_ORDER_FACT_ORATX_UPD"."GET_CONST_0_TRANSACTIONTYPE" ) ) )
...
BEGIN
  ...
  IF NOT (get_audit_level = AUDIT_NONE) THEN
    batch_auditd_id := WB_RT_MAPAUDIT.auditd_begin(  -- Template BatchAuditDetailBegin
      ...
      p_step=>0,
      p_name=>'"SALES_ORDER_FACT_Bat"',
      p_source=>'*',
      p_source_uoid=>'*',
      p_target=>'"SALES_ORDER_FACT"',
      p_target_uoid=>'A41FFB19C8385678E040007F01006C7D',
    ...
    p_parent_operator_uoid=>'A41FFB19C8975678E040007F01006C7D', -- Operator ORACLETX_HISTORY
    ...
    p_location_uoid=>'A41FFB18FCD75678E040007F01006C7D' -- Location SLXDW_SOURCE_LOCATION
    	...
    	get_audit_detail_type_id := WB_RT_MAPAUDIT.register_audit_detail_type(
    p_rtd=>get_audit_detail_id,
    p_parent_operator_uoid=>'A41FFB19C98C5678E040007F01006C7D', -- Operator SALES_ORDER_FACT
    ...
    p_location_uoid=>'A41FA16DADDC655CE040007F01006B9E' -- GKDW_TARGET_LOCATION
    	...
    	get_audit_detail_type_id := WB_RT_MAPAUDIT.register_audit_detail_type(
    p_rtd=>get_audit_detail_id,
    p_parent_operator_uoid=>'A41FFB19C8385678E040007F01006C7D', -- Operator SALES_ORDER_FACT
    p_parent_object_name=>'SALES_ORDER_FACT',
    p_parent_object_uoid=>'A41FA16DB216655CE040007F01006B9E', -- Table SALES_ORDER_FACT
    p_parent_object_type=>'Table',
    p_object_name=>'SALES_ORDER_FACT',
    ...
    p_location_uoid=>'A41FA16DADDC655CE040007F01006B9E' -- Location GKDW_TARGET_LOCATION
    	...
    	get_audit_detail_type_id := WB_RT_MAPAUDIT.register_audit_detail_type(
    p_rtd=>get_audit_detail_id,
    p_parent_operator_uoid=>'A41FFB19C6395678E040007F01006C7D', -- Operator SALES_ORDER_FACT_SOURCE
    p_parent_object_name=>'SALES_ORDER_FACT',
    p_parent_object_uoid=>'A41FFB1915405678E040007F01006C7D', -- Table SALES_ORDER_FACT
    p_parent_object_type=>'Table',
    p_object_name=>'SALES_ORDER_FACT',
    p_object_uoid=>'A41FFB1915405678E040007F01006C7D', -- Table SALES_ORDER_FACT
    p_object_type=>'Table',
    p_location_uoid=>'A41FFB18FD395678E040007F01006C7D' -- GKDW_SOURCE_LOCATION
  );
    	get_audit_detail_type_id := WB_RT_MAPAUDIT.register_audit_detail_type(
    p_rtd=>get_audit_detail_id,
    p_parent_operator_uoid=>'A41FFB19C8375678E040007F01006C7D', -- Operator SALES_ORDER_FACT_SOURCE
    p_parent_object_name=>'SALES_ORDER_FACT',
    p_parent_object_uoid=>'A41FFB1915405678E040007F01006C7D', -- Table SALES_ORDER_FACT
    p_parent_object_type=>'Table',
    p_object_name=>'SALES_ORDER_FACT',
...
USING
  (SELECT
  "SALES_ORDER_FACT"."SALES_ORDER_ID" "SALES_ORDER_ID$2",
  "ORACLETX_HISTORY"."CREATEDATE" "CREATEDATE"
FROM
    "SALES_ORDER_FACT"  "SALES_ORDER_FACT"   
 LEFT OUTER JOIN   "SLXDW"."ORACLETX_HISTORY"  "ORACLETX_HISTORY" ON ( ( ( "ORACLETX_HISTORY"."EVXEVENTID" = "SALES_ORDER_FACT"."SALES_ORDER_ID" ) ) AND ( ( "ORACLETX_HISTORY"."TRANSACTIONTYPE" = "OWB_SALES_ORDER_FACT_ORATX_UPD"."GET_CONST_0_TRANSACTIONTYPE" ) ) )
  ...
  ( "SALES_ORDER_FACT"."BILL_DATE" is null )
    ...
    MERGE_SUBQUERY
ON (
  "SALES_ORDER_FACT"."SALES_ORDER_ID" = "MERGE_SUBQUERY"."SALES_ORDER_ID$2"
   )
  
  WHEN MATCHED THEN
    UPDATE
    SET
                  "BILL_DATE" = "MERGE_SUBQUERY"."CREATEDATE"
  ...
  last_error_message := SQLERRM;
    ...
    IF get_audit_level = AUDIT_ERROR_DETAILS OR get_audit_level = AUDIT_COMPLETE THEN
      WB_RT_MAPAUDIT.error(
        ...
        p_rowkey=>0,
        p_table=>'"SALES_ORDER_FACT"',
        p_column=>'*',
        p_dstval=>NULL,
        p_stm=>'TRACE 289: ' || batch_action,
        p_sqlerr=>SQLCODE,
        p_sqlerrm=>SQLERRM,
        p_rowid=>NULL
      );
...
get_source_name            CONSTANT VARCHAR2(2000) := '"SALES_ORDER_FACT","SLXDW"."ORACLETX_HISTORY"';
get_source_uoid            CONSTANT VARCHAR2(2000) := 'A41FFB19C8375678E040007F01006C7D,A41FFB19C8975678E040007F01006C7D';
...
TYPE "T_FLTR_0_SALES_ORDER_ID" IS TABLE OF VARCHAR2(50) INDEX BY BINARY_INTEGER;
...
"SV_FLTR_0_SALES_ORDER_ID"  VARCHAR2(50);
...
"FLTR_0_SALES_ORDER_ID" "T_FLTR_0_SALES_ORDER_ID";
...
BEGIN
  ...
  WB_RT_MAPAUDIT.error_source(
    ...
    p_seq=>1,
    p_instance=>1,
    p_table=>SUBSTR('"SALES_ORDER_FACT","SLXDW"."ORACLETX_HISTORY"',0,80),
    p_column=>SUBSTR('FLTR_0_SALES_ORDER_ID',0,80),
    p_value=>SUBSTRB("FLTR_0_SALES_ORDER_ID"(error_index),0,2000),
...
BEGIN
  ...
  BEGIN
    "FLTR_0_SALES_ORDER_ID".DELETE;
    "ORACLETX_HISTORY_4_CREATEDATE".DELETE;

    FETCH
      "FLTR_c"
    BULK COLLECT INTO
      "FLTR_0_SALES_ORDER_ID",
      "ORACLETX_HISTORY_4_CREATEDATE"
    LIMIT get_bulk_size;

    IF "FLTR_c"%NOTFOUND AND "FLTR_0_SALES_ORDER_ID".COUNT = 0 THEN
      RETURN;
    END IF;
    -- register feedback for successful reads
    IF get_audit_level = AUDIT_ERROR_DETAILS OR get_audit_level = AUDIT_COMPLETE THEN
      get_rowkey := rowkey_counter;
      rowkey_counter := rowkey_counter + "FLTR_0_SALES_ORDER_ID".COUNT;
    END IF;
    ...
    END IF;
    get_map_selected := get_map_selected + "FLTR_0_SALES_ORDER_ID".COUNT;
  EXCEPTION
    WHEN OTHERS THEN
        last_error_number  := SQLCODE;
  last_error_message := SQLERRM;
...
PROCEDURE "FLTR_DML"(si NUMBER, firstround BOOLEAN) IS
  ...
  IF get_use_hc THEN
    ...
    get_row_status := TRUE;
  END IF;
  IF NOT "SALES_ORDER_FACT_St" AND (NOT get_use_hc OR get_row_status) 
  AND (NOT (get_use_hc AND "SALES_ORDER_FACT_nul")) THEN
    -- Update DML for "SALES_ORDER_FACT"
    ...
    IF get_use_hc AND NOT firstround THEN
      "SALES_ORDER_FACT_si" := "SALES_ORDER_FACT_ir"(si);
      IF "SALES_ORDER_FACT_si" = 0 THEN
        "SALES_ORDER_FACT_i" := 0;
      ELSE
        "SALES_ORDER_FACT_i" := "SALES_ORDER_FACT_si" + 1;
      END IF;
    ELSE
    ...
    LOOP
      ...
      EXIT WHEN "SALES_ORDER_FACT_i" <= get_bulk_size 
   AND "FLTR_c"%FOUND AND NOT get_abort;
      ELSE
        EXIT WHEN "SALES_ORDER_FACT_si" >= "SALES_ORDER_FACT_i";
      END IF;
      ...
      BEGIN
        FORALL i IN "SALES_ORDER_FACT_si".."SALES_ORDER_FACT_i" - 1
          ...
          SET
  					...
  					"SALES_ORDER_FACT"."BILL_DATE" = "SALES_ORDER_FACT_53_BILL_DATE"
  (i)
    ...
    IF NOT get_abort AND NOT get_abort_procedure THEN
      ...
      LOOP
        IF "FLTR_si" = 0 THEN
          ...
          IF NOT get_read_success THEN
            bulk_count := "FLTR_0_SALES_ORDER_ID".COUNT - 1;
          ELSE
            bulk_count := "FLTR_0_SALES_ORDER_ID".COUNT;
          END IF;
...
get_target_name := '"SALES_ORDER_FACT"';
            ...
            "FLTR_0_SALES_ORDER_ID"("FLTR_i");',0,2000);
            error_column := SUBSTRB('"SALES_OR_0_SALES_OR"',0,80);
            
            BEGIN
              error_value := SUBSTRB("FLTR_0_SALES_ORDER_ID"("FLTR_i"),0,2000);
            EXCEPTION 
              WHEN OTHERS THEN
                error_value := '*';
            END;
  ...
  last_error_message := SQLERRM;
                  ...
                  "FLTR_ER"('TRACE 294: ' || error_stmt, error_column, error_value, SQLCODE, SQLERRM, get_audit_detail_id, "FLTR_i");
        ...
        EXCEPTION
          WHEN OTHERS THEN
              last_error_number  := SQLCODE;
  last_error_message := SQLERRM;
            "FLTR_ER"('TRACE 292: ' || error_stmt, '*', NULL, SQLCODE, SQLERRM, NULL, "FLTR_i");
            get_errors := get_errors + 1;
...
-- Bulk: types for collection variables
TYPE "T_FLTR_0_SALES_ORDER_ID$1" IS TABLE OF VARCHAR2(50) INDEX BY BINARY_INTEGER;
...
-- Bulk: intermediate scalar variables
"SV_FLTR_0_SALES_ORDER_ID$1"  VARCHAR2(50);
...
-- Bulk: intermediate collection variables
"FLTR_0_SALES_ORDER_ID$1" "T_FLTR_0_SALES_ORDER_ID$1";
...
BEGIN
  ...
  WB_RT_MAPAUDIT.error_source(
    ...
    p_table=>SUBSTR('"SALES_ORDER_FACT","SLXDW"."ORACLETX_HISTORY"',0,80),
    p_column=>SUBSTR('FLTR_0_SALES_ORDER_ID',0,80),
    p_value=>SUBSTRB("FLTR_0_SALES_ORDER_ID$1"(error_index),0,2000),
    p_step=>get_step_number,
    p_role=>'S'
    );
  WB_RT_MAPAUDIT.error_source(
...
BEGIN
  ...
  BEGIN
    "FLTR_0_SALES_ORDER_ID$1".DELETE;
    "ORACLETX_HISTORY_4_CREATEDA".DELETE;

    FETCH
      "FLTR_c$1"
    BULK COLLECT INTO
      "FLTR_0_SALES_ORDER_ID$1",
      "ORACLETX_HISTORY_4_CREATEDA"
    LIMIT get_bulk_size;

    IF "FLTR_c$1"%NOTFOUND AND "FLTR_0_SALES_ORDER_ID$1".COUNT = 0 THEN
      RETURN;
    END IF;
    -- register feedback for successful reads
    IF get_audit_level = AUDIT_ERROR_DETAILS OR get_audit_level = AUDIT_COMPLETE THEN
      get_rowkey := rowkey_counter;
      rowkey_counter := rowkey_counter + "FLTR_0_SALES_ORDER_ID$1".COUNT;
    END IF;
    ...
    END IF;
    get_map_selected := get_map_selected + "FLTR_0_SALES_ORDER_ID$1".COUNT;
  EXCEPTION
    WHEN OTHERS THEN
        last_error_number  := SQLCODE;
  last_error_message := SQLERRM;
      ...
      BEGIN
        FORALL i IN "SALES_ORDER_FACT_si".."SALES_ORDER_FACT_i" - 1
          ...
          SET
  					...
  					"SALES_ORDER_FACT"."BILL_DATE" = "SALES_ORDER_FACT_53_BILL_DA"
  (i)
    ...
    IF NOT get_abort AND NOT get_abort_procedure THEN
      ...
      LOOP
        IF "FLTR_si$1" = 0 THEN
          ...
          IF NOT get_read_success THEN
            bulk_count := "FLTR_0_SALES_ORDER_ID$1".COUNT - 1;
          ELSE
            bulk_count := "FLTR_0_SALES_ORDER_ID$1".COUNT;
          END IF;
...
get_target_name := '"SALES_ORDER_FACT"';
            ...
            "FLTR_0_SALES_ORDER_ID$1"("FLTR_i$1");',0,2000);
            error_column := SUBSTRB('"SALES_OR_0_SALES_OR$1"',0,80);
            
            BEGIN
              error_value := SUBSTRB("FLTR_0_SALES_ORDER_ID$1"("FLTR_i$1"),0,2000);
            EXCEPTION 
              WHEN OTHERS THEN
                error_value := '*';
            END;
            ...
            "FLTR_0_SALES_ORDER_ID$1"("FLTR_i$1");
  ...
  last_error_message := SQLERRM;
                  ...
                  "FLTR_ER$1"('TRACE 299: ' || error_stmt, error_column, error_value, SQLCODE, SQLERRM, get_audit_detail_id, "FLTR_i$1");
        ...
        EXCEPTION
          WHEN OTHERS THEN
              last_error_number  := SQLCODE;
  last_error_message := SQLERRM;
            "FLTR_ER$1"('TRACE 297: ' || error_stmt, '*', NULL, SQLCODE, SQLERRM, NULL, "FLTR_i$1");
            get_errors := get_errors + 1;
            IF get_errors > get_max_errors THEN  
  get_abort := TRUE;
END IF;
...
BEGIN
  ...
  IF NOT (get_audit_level = AUDIT_NONE) THEN
    get_runtime_audit_id := WB_RT_MAPAUDIT.audit_begin(  -- Template AuditBegin
      ...
      p_parent=>NULL,
      p_source=>'"SALES_ORDER_FACT","SLXDW"."ORACLETX_HISTORY"',
      p_source_uoid=>'A41FFB19C8375678E040007F01006C7D,A41FFB19C8975678E040007F01006C7D',
      p_target=>'"SALES_ORDER_FACT"',
      p_target_uoid=>'A41FFB19C8385678E040007F01006C7D',      p_info=>NULL,
...
BEGIN
  ...
  IF get_operating_mode = MODE_SET THEN
    ...
    IF NOT get_use_hc OR get_batch_status THEN
      "SALES_ORDER_FACT_St" := "SALES_ORDER_FACT_Bat";
      get_batch_status := get_batch_status AND "SALES_ORDER_FACT_St";
    END IF;
...
BEGIN
  ...
  AND    uo.object_name = 'OWB_SALES_ORDER_FACT_ORATX_UPD'
  AND    uo.object_id = ao.object_id;

  wb_rt_mapaudit_util.premap('OWB_SALES_ORDER_FACT_ORATX_UPD', x_schema, x_audit_id, x_object_id);
...
END "OWB_SALES_ORDER_FACT_ORATX_UPD";
/
