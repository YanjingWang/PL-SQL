DROP MATERIALIZED VIEW GKDW.GK_SPEND_REPORT_MV;
CREATE MATERIALIZED VIEW GKDW.GK_SPEND_REPORT_MV 
TABLESPACE GDWSML
PCTUSED    0
PCTFREE    10
INITRANS   2
...
WITH PRIMARY KEY
AS 
/* Formatted on 29/01/2021 12:21:50 (QP5 v5.115.810.9015) */
SELECT   f.enroll_id,
         f.event_id,
         f.cust_id,
         cd.ch_num,
         ...
         cp.ob_comm_type,
         sl.segment,
         sl.field_rep sl_field_rep,
         sl.ob_terr sl_ob_terr,
         sl.field_terr sl_field_terr,
         gt.salesrep zip_field_rep,
  ...
  FROM                                                                  order_fact f
                     ...
                     LEFT OUTER JOIN
                        slxdw.userinfo ui3
                     ON f.salesperson = ui3.username
                        AND ui3.region != 'Retired'
                  LEFT OUTER JOIN
 ...
 WHERE   td1.fiscal_year >= 2013 AND f.book_amt <> 0;

COMMENT ON MATERIALIZED VIEW GKDW.GK_SPEND_REPORT_MV IS 'snapshot table for snapshot GKDW.GK_SPEND_REPORT_MV';

GRANT SELECT ON GKDW.GK_SPEND_REPORT_MV TO DWHREAD;
