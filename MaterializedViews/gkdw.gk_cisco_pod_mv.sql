DROP MATERIALIZED VIEW GKDW.GK_CISCO_POD_MV;
CREATE MATERIALIZED VIEW GKDW.GK_CISCO_POD_MV 
TABLESPACE GDWSML
PCTUSED    0
PCTFREE    10
INITRANS   2
...
WITH PRIMARY KEY
AS 
/* Formatted on 29/01/2021 12:25:57 (QP5 v5.115.810.9015) */
SELECT   t1.enroll_id,
         t1.event_id,
         t1.enroll_date,
         t1.keycode,
 ...
 WHERE       t6.dim_year = 2009
         AND t3.course_pl = 'CISCO'
         AND t3.course_ch = 'INDIVIDUAL/PUBLIC'
         AND t3.course_mod IN ('C-LEARNING', 'RESELLER - C-LEARNING')
         AND NVL (t8.territory_type, 'OB') = 'OB';

COMMENT ON MATERIALIZED VIEW GKDW.GK_CISCO_POD_MV IS 'snapshot table for snapshot GKDW.GK_CISCO_POD_MV';

GRANT SELECT ON GKDW.GK_CISCO_POD_MV TO DWHREAD;
