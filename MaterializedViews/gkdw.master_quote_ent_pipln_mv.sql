DROP MATERIALIZED VIEW GKDW.MASTER_QUOTE_ENT_PIPLN_MV;
CREATE MATERIALIZED VIEW GKDW.MASTER_QUOTE_ENT_PIPLN_MV 
TABLESPACE GDWSML
PCTUSED    0
PCTFREE    10
INITRANS   2
...
WITH PRIMARY KEY
AS 
/* Formatted on 29/01/2021 12:20:06 (QP5 v5.115.810.9015) */
SELECT   ac.accountid,
         ...
         d.actual_amount,
         CASE
            WHEN     sp.processtype = 'STAGE'
                 AND sp.stagename = 'Discovery'
                 AND UPPER (ui4.department) LIKE '%ENTERPRISE%'
            THEN
               d.sales_potential
            ELSE
               NULL
  ...
  FROM                                          opportunity_dim d
                              ...
                              LEFT OUTER JOIN
                                 gk_territory gt
                              ON ad1.postalcode BETWEEN gt.zip_start
                                                    AND  gt.zip_end
            ...
            LEFT OUTER JOIN
               slxdw.userinfo ui4
            ON d.account_manager_id = ui4.userid
         LEFT OUTER JOIN
            (SELECT   entityid,
                      completeddate,
                      processtype,
                      stagename
               FROM   salesprocessaudit@slx) sp
         ON     d.opportunity_id = sp.entityid
            --       inner join opportunity_dim d1
            --       on d1.opportunity_id = sp.entityid
            AND TRUNC (sp.completeddate) = td1.dim_date
            AND sp.processtype = 'STAGE'
            AND sp.stagename = 'Discovery'
 WHERE   td1.fiscal_year >= 2015;

COMMENT ON MATERIALIZED VIEW GKDW.MASTER_QUOTE_ENT_PIPLN_MV IS 'snapshot table for snapshot GKDW.MASTER_QUOTE_ENT_PIPLN_MV';

GRANT SELECT ON GKDW.MASTER_QUOTE_ENT_PIPLN_MV TO DWHREAD;
