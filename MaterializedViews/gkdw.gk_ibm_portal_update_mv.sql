DROP MATERIALIZED VIEW GKDW.GK_IBM_PORTAL_UPDATE_MV;
CREATE MATERIALIZED VIEW GKDW.GK_IBM_PORTAL_UPDATE_MV 
TABLESPACE GDWSML
PCTUSED    0
PCTFREE    10
INITRANS   2
...
WITH PRIMARY KEY
AS 
/* Formatted on 29/01/2021 12:24:42 (QP5 v5.115.810.9015) */
SELECT   ed.event_id event_schedule_id,
 ...
 WHERE       ed.status = 'Open'
         AND cd.ch_num = '10'
         AND ed.last_update_date >= TRUNC (SYSDATE) - 1
UNION
SELECT   ed.event_id event_schedule_id,
         cd.course_code,
         ed.event_id,
...
COMMENT ON MATERIALIZED VIEW GKDW.GK_IBM_PORTAL_UPDATE_MV IS 'snapshot table for snapshot GKDW.GK_IBM_PORTAL_UPDATE_MV';

GRANT SELECT ON GKDW.GK_IBM_PORTAL_UPDATE_MV TO DWHREAD;
