DROP PROCEDURE GKDW.GK_CCNA_USER_GEN;

CREATE OR REPLACE PROCEDURE GKDW.gk_ccna_user_gen(p_date date default null) as
cursor c1(pp_date date) is
  ...
  from event_dim ed
       inner join lms_import_param ip on ed.course_code = ip.course_code and ip.user_group in ('CCNA Bootcamp')
...
if c1%found then
  close c1;
  select 'user_info.xml','/mnt/nc10s210/user_info.xml'
      into xml_file_name,xml_file_name_full
      from dual;

  xml_file := utl_file.fopen('/mnt/nc10s210',xml_file_name,'w');
