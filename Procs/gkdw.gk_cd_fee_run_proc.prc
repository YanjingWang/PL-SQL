DROP PROCEDURE GKDW.GK_CD_FEE_RUN_PROC;

CREATE OR REPLACE PROCEDURE GKDW.gk_cd_fee_run_proc as

cursor c1 is
select dim_period_name,max(dim_date) dim_date
  from time_dim td
 where td.dim_date >= trunc(sysdate)
...
open c1; fetch c1 into r1;
  if c1%found and r1.dim_date = trunc(sysdate) then
    gk_cd_fee_proc(r1.dim_period_name);
