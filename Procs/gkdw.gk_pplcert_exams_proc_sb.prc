DROP PROCEDURE GKDW.GK_PPLCERT_EXAMS_PROC_SB;

CREATE OR REPLACE PROCEDURE GKDW.gk_pplCert_exams_proc_sb is

--PEOPLECERT/PPLCERT VOUCHER EVENT DATA SET
cursor c0 is
select 16879 requestor_id, --Jamie Clark
       536001 vendor_id,'REMIT NICOSIA' vendor_site_code,84 v_org_id,101 v_inv_org_id,
       ...
       'PEOPLECERT Exam '||to_char(ed.start_date,'mm/dd/yy')||' '||cd.course_code||' '||ed.city||', '||ed.state po_line_desc,
       ...
       'N' retake_flag,
       'PEOPLECERT EXAMS'||chr(9)||cd.course_code||chr(9)||cd.short_name||chr(9)||ed.facility_region_metro||chr(9)||
...
--PEOPLECERT/PPLCERT VOUCHER DETAIL DATA SET   
...
select ed.ops_country,ed.event_id,cd.vendor_code,cd.course_code,cd.short_name,ed.facility_region_metro,
       ...
       ct.firstname customer_firstname,ct.lastname customer_lastname,c.acct_name,c.address1,c.address2,c.city,c.state,c.zipcode,c.country,
       ...
       'PEOPLECERT EXAMS-'||chr(9)||ed.event_id||'-'||cd.course_code||'-'||c.cust_name po_line_desc,
       td.dim_year||'-'||lpad(td.dim_week,2,'0')||chr(9)||ed.event_id||chr(9)||'PEOPLECERT EXAMS'/*cd.vendor_code*/||chr(9)||cd.course_code||chr(9)||cd.short_name||chr(9)||
       ed.facility_region_metro||chr(9)||ed.start_date||chr(9)||ed.end_date||chr(9)||ie.firstname||' '||ie.lastname||chr(9)||ie.feecode||chr(9)||
       ct.firstname||chr(9)||ct.lastname||chr(9)||c.acct_name||chr(9)||c.address1||chr(9)||c.address2||chr(9)||c.city||chr(9)||c.state||chr(9)||c.zipcode||chr(9)||
  ...
  from event_dim ed
       ...
       left outer join slxdw.contact ct on f.cust_id = ct.contactid
       left outer join time_dim td on ed.start_date = td.dim_date
 where ed.event_id = v_event_id;

--PEOPLECERT/PPLCERT VOUCHER FUTURE EVENT DATA SET
--cursor c2 is
...
--  from event_dim ed
--       inner join course_dim cd on ed.course_id = cd.course_id and ed.ops_country = cd.country
--       inner join gk_PEOPLECERT_courses cp on cd.course_code = cp.course_code and cp.voucher_flag = 'Y'
--       inner join instructor_event_v ie on ed.event_id = ie.evxeventid and ie.feecode in ('INS','SI')
--       inner join time_dim td1 on ed.start_date = td1.dim_date
--       inner join time_dim td2 on td2.dim_date = trunc(sysdate)
--       inner join slxdw.evxfacility f on ed.location_id = f.evxfacilityid
...
--WEEKLY PEOPLECERT EXAM VOUCHER PROCESS
select 'PeopleCert_exam_'||to_char(sysdate-5,'yyyymmddhh24miss')||'.xls',
       '/usr/tmp/PeopleCert_exam_'||to_char(sysdate-5,'yyyymmddhh24miss')||'.xls',
       'PeopleCert_exam_detail_'||to_char(sysdate-5,'yyyymmddhh24miss')||'.xls',
       '/usr/tmp/PeopleCert_exam_detail_'||to_char(sysdate-5,'yyyymmddhh24miss')||'.xls'
...
select distinct td.dim_year||'-'||lpad(td.dim_week,2,'0') into v_po_week
 ...
 where td1.dim_date = trunc(sysdate-5);
...
v_detail_hdr := v_detail_hdr||chr(9)||'Instructor'||chr(9)||'Feecode'||chr(9)||'Cust First Name'||chr(9)||'Cust Last Name'||chr(9)||'Acct Name'||chr(9)||'Address1'||chr(9)||'Address2'||chr(9)||'City';
...
for r0 in c0 loop
  ...
  send_mail('DW.Automation@globalknowledge.com','smaranika_baral@globalknowledge.com','Cur 1 completed','Cur 1 done');
  ...
  for r1 in c1(r0.event_id) loop
    ...
    send_mail('DW.Automation@globalknowledge.com','smaranika.baral@globalknowledge.com','Cur 2 completed','Cur 2 done');
...
--  end if;
  ...
  insert into gk_itp_exam_audit
    select r0.event_id,v_curr_po,r0.enroll_cnt,r0.unit_price,sysdate-5,r0.curr_code from dual;
  commit;
...
send_mail('DW.Automation@globalknowledge.com','smaranika.baral@globalknowledge.com','send file','sent');
...
v_error:= SendMailJPkg.SendMail(
          ...
          CcRecipient => '',
          BccRecipient => '',
          Subject   => 'PEOPLECERT EXAM PO FOR WEEK OF '||v_po_week,
...
exception
  when others then
    rollback;
    send_mail('DW.Automation@globalknowledge.com','DW.Automation@globalknowledge.com','GK_PPLCERT_EXAMS_PROC Failed',SQLERRM);
