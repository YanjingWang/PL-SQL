DROP PROCEDURE GKDW.ORDER_FACT_ADDR_UPD_PROC_TEST;

CREATE OR REPLACE PROCEDURE GKDW.order_fact_addr_upd_proc_test as
cursor c1 is
select distinct f.enroll_id,
--f.creation_date,f.last_update_date,f.enroll_id,f.acct_id,f.cust_id,upper(ac.account) acct_name,upper(ad1.address1) acct_address1,upper(ad1.address2) acct_address2,upper(ad1.city) acct_city,upper(ad1.state) acct_state,ad1.postalcode acct_zipcode,
--       upper(ad1.country) acct_country,upper(ad1.county) acct_county,
upper(c.firstname) cust_first_name,upper(c.lastname) cust_last_name,lower(c.email) cust_email--,upper(ad2.address1) cust_address1,
--       upper(ad2.address2) cust_address2,upper(ad2.city) cust_city,upper(ad2.state) cust_state,ad2.postalcode cust_zipcode,upper(ad2.country) cust_country,upper(ad2.county) cust_county
  from order_fact f
       ...
       inner join TBD_ONSITE_CLEANUP@slx_imp tc on f.enroll_id = tc.evxevenrollid;
...
--       inner join slxdw.address ad1 on ac.addressid = ad1.addressid
--       inner join TBD_ONSITE_CLEANUP@slx_imp tc on f.enroll_id = tc.evxevenrollid
-- where rev_date >= trunc(sysdate)-1 or book_date >= trunc(sysdate)-1
--minus
