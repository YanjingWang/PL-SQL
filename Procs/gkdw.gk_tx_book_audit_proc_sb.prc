DROP PROCEDURE GKDW.GK_TX_BOOK_AUDIT_PROC_SB;

CREATE OR REPLACE PROCEDURE GKDW.gk_tx_book_audit_proc_sb(p_bookdate varchar2 default null) as
...
for r0 in c0(v_bookdate) loop
  ...
  if r0.source in ('CA_OPS','CA_NEST') then
     ...
     send_mail('DW.Automation@globalknowledge.com','smaranika.baral@globalknowledge.com',v_mail_hdr,v_msg_body);
  else
   send_mail('DW.Automation@globalknowledge.com','smaranika.baral@globalknowledge.com',v_mail_hdr,v_msg_body);
