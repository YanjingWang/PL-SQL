DROP PROCEDURE GKDW.GK_MYGK_INSTR_EMAIL_PROC;

CREATE OR REPLACE PROCEDURE GKDW.gk_mygk_instr_email_proc as
...
select distinct ie.contactid,ie.firstname||' '||ie.lastname inst_name,ie.email inst_email
 ...
 where ed.status = 'Open'
   and ed.start_date = trunc(sysdate)+1
...
select ed.event_id,ed.facility_region_metro,ed.location_name,ed.facility_code,
  ...
  from event_dim ed
       ...
       inner join gk_mygk_course_profile_v p on cd.course_id = p.course_id
 where ed.start_date = trunc(sysdate)+1
...
for r0 in c0 loop
  ...
  if r3.virtual_flag = 'Y' then
    ...
    v_msg_body := v_msg_body||'<tr><td align=left>As you are teaching a virtual or connected (classroom and virtual) event, access your Zoom session is through the link below.</td></tr>';
    v_msg_body := v_msg_body||'<tr><td align=left>https://globalknowledge.zoom.us</td></tr>';
...
GRANT EXECUTE ON GKDW.GK_MYGK_INSTR_EMAIL_PROC TO COGNOS_RO;
