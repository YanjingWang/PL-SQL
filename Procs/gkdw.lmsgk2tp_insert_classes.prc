DROP PROCEDURE GKDW.LMSGK2TP_INSERT_CLASSES;

CREATE OR REPLACE PROCEDURE GKDW.LMSGK2TP_insert_classes  as 
...
cursor c1 is 
    ...
    substr(ed.COURSE_CODE, -1, 1) in ('A', 'C', 'G', 'L','U', 'W')
    and ed.START_DATE >= '01-JUL-2017'
    and ed.COUNTRY in ( 'CANADA', 'USA')
    --and (ed.CREATION_DATE >= '17-NOV-2012' or ed.LAST_UPDATE_DATE >= '17-NOV-2012')
    --and (ed.CREATION_DATE >= LAST_RUN_DATE_OUT or ed.LAST_UPDATE_DATE >= LAST_RUN_DATE_OUT)
    and (trunc(ed.GKDW_CREATE_DATE) >= LAST_RUN_DATE_OUT or trunc(ed.GKDW_UPDATE_DATE) >= LAST_RUN_DATE_OUT)
    and event_id not in ('Q6UJ9A07TIVF')
union
    select distinct
        ed.COURSE_ID COURSE_NO_GK ,
        ed.LOCATION_ID FACILITY_NO_GK,
    ...
    and (substr(ed.COURSE_CODE, -1, 1) = 'V' 
     or ED.COURSE_CODE in ( '2005Y','9774Y')) -- Added on 10/09/2014 by Sumitra - per Freds request.
...
union
    ...
    where --cd.INACTIVE_FLAG = 'F'    and /*** Inactive Flag commented out - 02/04/2013   
    substr(ed.COURSE_CODE, -1, 1) in ( 'N', 'H')
    and ed.START_DATE >= '01-JUL-2017'
...
begin
   ...
   select nvl(trunc(last_run_date), '01-JAN-2009') into last_run_date_out from LMSGK2TP_TIMESTAMP@dx_prod ;

--select trunc(sysdate) into last_run_date_out from dual;
...
DBMS_OUTPUT.Put_Line('in loop...');
    ...
    DBMS_OUTPUT.Put_Line('after fetch...' || r1.event_no);
