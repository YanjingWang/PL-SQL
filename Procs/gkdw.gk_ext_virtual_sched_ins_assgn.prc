DROP PROCEDURE GKDW.GK_EXT_VIRTUAL_SCHED_INS_ASSGN;

CREATE OR REPLACE PROCEDURE GKDW.gk_ext_virtual_sched_INS_ASSGN as
...
select vs.event_id,substr(l.country,1,2) country,s."id" schedule_id,
 ...
 where l.course_code = vs.course_code 
   and to_date(l.day1,'MM/DD/YYYY') = vs.day1 
   and to_date(l.lastday,'mm/dd/yyyy') = vs.lastday
   and to_char(to_date(l.start_time,'hh24:mi'),'fmhhfm:mi AM') = vs.start_time
