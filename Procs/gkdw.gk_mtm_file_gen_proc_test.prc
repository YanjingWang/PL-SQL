DROP PROCEDURE GKDW.GK_MTM_FILE_GEN_PROC_TEST;

CREATE OR REPLACE PROCEDURE GKDW.gk_mtm_file_gen_proc_TEST(p_date date default null,p_test varchar2 default 'N') as
cursor c0(vv_date date) is
  select ops_country,
         'TEST_MTM_GK_'||substr(ops_country,1,2)||'_'||to_char(sysdate,'yyyy-mm-dd-hh24-mi')||'.xml' fname,
...
cursor c2(t_date date,v_ops_country varchar2) is
  select distinct 
      chr(9)||'<course xid="'||xml_course_id||'" name="'||
         case
            when course_mod in ('SPeL CD','RESELLER - SPeL') then 
               case when CCL.description is not null then CCL.description else MTM.course_name end 
            else trim(regexp_replace(replace(
                                              case when CCL.description is not null then CCL.description else  mtm_course_title end 
                ,chr(38),chr(38)||'amp;'), '[^ -~]', '' ))
         end ||'" desc="'||
         case
            when course_mod in ('SPeL CD','RESELLER - SPeL') then 
    ...
    from gk_mtm_data_mv MTM
	LEFT OUTER JOIN CISCO_COURSE_LOOKUP CCL ON regexp_replace(short_name, 'v[0-9]',  substr(regexp_substr( short_name, 'v[0-9]'), - 1) ) = CCL.COURSE_NAME
   where survey_date = t_date
     and ops_country = v_ops_country;
...
cursor c4(t_date date,v_ops_country varchar2) is
   select distinct chr(9)||'<class xid="'||
    ...
    left outer join (SELECT fl.survey_id,
       fl.pl_desc,
       fl.pl_type,
       fl.md_desc,
       fl.loc_type,
...
cursor c5(v_event_id varchar2) is
  select chr(9)||chr(9)||'<student xid="'||f.cust_id||'" email="'||
         ...
         trim(regexp_replace(replace(replace(replace(replace(replace(replace(substr(cd.acct_name,1,50),chr(38),chr(38)||'amp;'),chr(201),chr(69)),chr(191),''), chr(158)), chr(148)), chr(153)), '[^ -~]', ' ' )) ||'" jt="8" />' stud_xml_line
...
if p_test = 'Y' then
  v_error:= SendMailJPkg.SendMail(
            ...
            Sender    => 'JAMAAL.JONES@globalknowledge.com',
            Recipient => 'JAMAAL.JONES@globalknowledge.com',
            CcRecipient => '',
            BccRecipient => '',
            Subject   => r0.fname,
            Body => 'TEST - MTM XML File is Attached.',
...
/*else
  v_error:= SendMailJPkg.SendMail(
            ...
            Recipient => 'amy.dethlefsen@globalknowledge.com',
            CcRecipient => '',
            BccRecipient => '',
            Subject   => r0.fname,
            Body => 'TEST - MTM XML File is Attached.',
