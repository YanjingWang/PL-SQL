DROP PROCEDURE GKDW.GK_GO_NOGO_PROC_BACKUP;

CREATE OR REPLACE PROCEDURE GKDW.gk_go_nogo_proc_backup as


--  =======================================================================
--  PROGRAM NAME : gk_go_nogo
--  PURPOSE      : To Populate the GNG Report
--  Note         :
--
--  UPDATE HISTORY
...
--  -----------  -----    ---------------------    -------------
--  23-Jul-2007  1.0                               New
--  28-Apr-2020  1.1      Smaranika Baral         Ticket #26971: GNG Cost initiative - SQL Server Report Correction USD and CAD currencies. 
--  13-May-2020  1.2      Smaranika Baral         Ticket #27808: Go No Go Report Issue- Student Count Discrepancy 
-- =======================================================================


--Ticket #26971 : Cursor Modified
...
cursor c0 is
  ...
  select event_id,rev_amt
    from gk_go_nogo) group by event_id;
    
 --Ticket #27808: Added the cursor for Enroll_cnt   
...
cursor c_enroll_cnt is  
   select l.event_id, sum(case when f.enroll_id is not null and f.book_amt > 0 then 1 else 0 end) enroll_cnt,
    ...
    from gk_go_nogo l
         left outer join order_fact f on l.event_id = f.event_id and f.enroll_status != 'Cancelled'
  where l.end_date >= trunc(sysdate-21)
   group by l.event_id
  minus
...
-- Ticket #27808: End
----Ticket #26971 : C0_6 Cursor Modified
cursor c0_6 is

 select event_id,sum(rev_amt) rev_amt 
 from ( select distinct l.event_id,
                        (case when curr_code is not null then round(sum(nvl(book_amt*rate,0)),2) else sum(nvl(book_amt,0)) end )  rev_amt
    from gk_go_nogo l
...
select td.dim_year||'-'||lpad(td.dim_week,2,'0') start_week,
       ...
       (case when curr_code is not null then round(sum(nvl(book_amt*rate,0)),2) else sum(nvl(book_amt,0)) end ) rev_amt,--Ticket #26971 
...
/**** Ticket #27808 : ENROLL COUNT UPDATES ****/
