DROP PROCEDURE GKDW.GK_GILMORE_CISCO_PO_PROC_BKP;

CREATE OR REPLACE PROCEDURE GKDW.gk_gilmore_cisco_po_proc_bkp(p_test varchar2 default 'N') as
...
8150 vendor_id,'CISCO' vendor_site_code,84 org_id,101 inv_org_id,4770 agent_id,10690 requestor,'USD' curr_code
 ...
 where f.enroll_status not in ('Cancelled','Did Not Attend')
  and trunc(f.creation_date) = trunc(sysdate)-1
     and f.book_amt > 0
...
--  end if;
    ...
    select v_curr_po,r1.vendor_id,r1.vendor_site_code,r1.org_id,r1.agent_id,r1.curr_code,v_sid,sysdate,'N','GILMORE CISCO'
      from dual;
...
--    if p_test = 'Y' then -- SR 06/04/2019 comment IF as R12DEV is down
...
if p_test = 'Y' then
      v_error:= SendMailJPkg.SendMail(
            ...
            BccRecipient => '',
            Subject   => 'GILMORE CISCO PO '||to_char(sysdate,'dd-Mon-yy'),
      ...
      v_error:= SendMailJPkg.SendMail(
            SMTPServerName => 'corpmail.globalknowledge.com',
            Sender    => 'dw.automation@globalknowledge.com',
            Recipient => 'AccountsPayable@globalknowledge.com',
...
exception
  when others then
  rollback;
    send_mail('DW.Automation@globalknowledge.com','DW.Automation@globalknowledge.com','gk_gilmore_cisco_po_proc failed',SQLERRM);
     send_mail('DW.Automation@globalknowledge.com','bala.subramanian@globalknowledge.com','gk_gilmore_cisco_po_proc failed',SQLERRM);
     send_mail('DW.Automation@globalknowledge.com','Smaranika.baral@globalknowledge.com','gk_gilmore_cisco_po_proc failed',SQLERRM);
