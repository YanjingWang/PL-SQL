DROP PROCEDURE GKDW.GK_GO_NOGO_PROC_BACK_UP;

CREATE OR REPLACE PROCEDURE GKDW.gk_go_nogo_proc_back_up as

cursor c0 is
 select event_id,sum(rev_amt) rev_amt
  from 
  (  select distinct l.event_id,
    (case when curr_code is not null then round(sum(nvl(book_amt*rate,0)),2) else sum(nvl(book_amt,0)) end )  rev_amt
    from gk_go_nogo_sb l
         left outer join order_fact f on l.event_id = f.event_id and f.enroll_status != 'Cancelled'
    left outer join fx_rate_sb on curr_code=currency_code
    where l.end_date >= trunc(sysdate-20)
   group by l.event_id,curr_code
  minus
  select event_id,rev_amt
    from gk_go_nogo_sb) group by event_id;
    
    
  cursor c_enroll_cnt is  
   select l.event_id, sum(case when f.enroll_id is not null and f.book_amt > 0 then 1 else 0 end) enroll_cnt,
  ...
  where l.end_date >= trunc(sysdate-20)
   group by l.event_id
  minus
  select event_id,enroll_cnt,guest_cnt,allaccess_cnt
    from gk_go_nogo_sb;

cursor c0_6 is
  select distinct l.event_id, rev_amt 
    from gk_go_nogo_sb l
...
cursor c0_loc is
  select ed.event_id,ed.location_id,ed.city,ed.state,ed.zipcode,ed.facility_region_metro,ed.facility_code,
    ...
    from event_dim ed
         ...
         inner join gk_go_nogo_sb l on ed.event_id = l.event_id
...
select l.event_id nested_event,l2.event_id master_event
  from gk_go_nogo_sb l
       ...
       inner join gk_go_nogo_sb l2 on nc.master_course_code = l2.course_code
...
cursor c24 is
  select l.event_id connected_event,l2.event_id master_event
    from gk_go_nogo_sb l
         ...
         inner join gk_go_nogo_sb l2 on c."parent_evxeventid" = l2.event_id
   where l.end_date >= trunc(sysdate);
...
select l2.event_id,
  ...
  from gk_go_nogo_sb l1
       inner join gk_go_nogo_sb l2 on l1.end_date+1 = l2.start_date
...
select i.event_id, i.enroll_cnt, i.start_date, ed.meeting_days
  from gk_go_nogo_sb i LEFT JOIN event_dim ed
...
execute immediate 'truncate table gk_go_nogo_curr';

insert into gk_go_nogo_curr
(start_week,event_id,course_id,location_id,start_date,end_date,event_city,event_state,event_zipcode,
 ...
 md_num,pl_num,short_name,rev_amt,enroll_cnt,unlimited_cnt,nex_enroll_cnt,gk_enroll_cnt,guest_cnt,allaccess_cnt,curr_code)
select td.dim_year||'-'||lpad(td.dim_week,2,'0') start_week,
       ...
       sum(case when f.enroll_id is not null and f.book_amt = 0 and f.keycode NOT IN  ('ALLACCESS','CISCOELITE','CISCOPREMIER') then 1 else 0 end) guest_cnt,
       sum(case when f.enroll_id is not null and f.book_amt = 0 and f.keycode IN ('ALLACCESS','CISCOELITE','CISCOPREMIER') then 1 else 0 end) allaccess_cnt,curr_code
  from event_dim ed
       inner join course_dim cd on ed.course_id = cd.course_id and ed.ops_country = cd.country
       inner join time_dim td on ed.start_date = td.dim_date
 ...
 where td.dim_date between trunc(sysdate) and trunc(sysdate)+120
   and ed.status != 'Cancelled'
   --and ed.event_id='QGKID0B876QX'
  -- and ed.start_date between  '1-May-2020' and '9-May-2020'
 --  and ed.event_id in ( select distinct event_id from gk_go_nogo_sb where start_date between '1-May-2020' and '9-May-2020')
  -- and ed.start_date between  '1-May-2020' and '9-May-2020'
   and cd.ch_num = '10'
   and cd.md_num in ('10','20')
   and cd.pl_num != '01'
   and not exists (select 1 from gk_go_nogo_sb l where ed.event_id = l.event_id)
 group by td.dim_year||'-'||lpad(td.dim_week,2,'0'),
          ...
          cd.course_ch,cd.course_mod,cd.course_pl,cd.ch_num,cd.md_num,cd.pl_num,cd.short_name,ed.ops_country,curr_code
 order by facility_region_metro,facility_code,course_pl;
 
 insert into gk_go_nogo_sb
(start_week,event_id,course_id,location_id,start_date,end_date,event_city,event_state,event_zipcode,
 facility_region_metro,facility_code,ops_country,course_code,course_ch,course_mod,course_pl,ch_num,
...
for r_enroll_cnt in c_enroll_cnt loop

update gk_go_nogo_sb
     set enroll_cnt = r_enroll_cnt.enroll_cnt,
         gk_enroll_cnt = r_enroll_cnt.enroll_cnt,
         guest_cnt = r_enroll_cnt.guest_cnt,
         allaccess_cnt = r_enroll_cnt.allaccess_cnt
   where event_id = r_enroll_cnt.event_id;
end loop;
