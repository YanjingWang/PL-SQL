DROP PROCEDURE GKDW.GK_CD_FEE_PROC_SB;

CREATE OR REPLACE PROCEDURE GKDW.gk_cd_fee_proc_sb(p_name varchar2) as
...
select cd.course_code,cf.primary_poc contact_name,cf.primary_poc_email contact_email,cf.vendor_name contact_company,cf.vendor_num,cf.vendor_name,
 ...
 where td.dim_period_name = upper(p_name)
   and trunc(sysdate-7) between nvl(cf.from_date,sysdate-7) and nvl(cf.to_date,sysdate-7)
  ...
  from gk_course_director_fees_mv cf
       ...
       inner join po_vendor_sites_all@r12prd vs on v.vendor_id = vs.vendor_id and upper(cf.vendor_site_code) = vs.vendor_site_code
                 -- and case when (substr(ed.ops_country,1,2) = 'CA' and v.vendor_id != 819002) then 86 else 84 end = vs.org_id
                  and case when substr(ed.ops_country,1,2) = 'CA' 
                           and v.vendor_id not in (select vendor_id from po_vendors@r12prd where vendor_id = '819002') then '86' else '84' end = vs.org_id
       left outer join per_all_people_f@r12prd pp on upper(cf.pm_email) = upper(pp.email_address) 
                                                  and pp.effective_end_date >= trunc(sysdate-7) and pp.current_employee_flag = 'Y'
 where td.dim_period_name = upper(p_name)
   and trunc(sysdate-7) between nvl(cf.from_date,sysdate-7) and nvl(cf.to_date,sysdate-7)
   and td.dim_date not in (to_date('02/26/2018','mm/dd/yyyy'),to_date('02/27/2018','mm/dd/yyyy')) -- SR 03/19/2018 This can be removed after 3/28/2018 
   and cf.fee_status = 'Active'
...
select distinct h.segment1,h.po_header_id,l.po_line_id,h.vendor_id,h.vendor_site_id,
       d.deliver_to_person_id,trunc(sysdate-7) ship_date,d.destination_organization_id,l.category_id,
       l.line_num,l.item_description,l.quantity,trunc(sysdate-7) deliv_date,
...
for r1 in c1 loop
  ...
  insert into gk_autoreceive_pos
    select curr_po,r1.vendor_id,r1.vendor_site_code,r1.org_id,r1.agent_id,r1.currency,v_sid,sysdate-7,'N','COURSE DIRECTOR FEE'
      from dual;
  commit;
...
send_mail_attach('DW.Automation@globalknowledge.com','Al.Koltas@globalknowledge.com ',null,'Michalina.DeBoard@globalknowledge.com',v_mail_hdr,'CD Fee PO Report',v_file_name_full_po);
...
send_mail_attach('DW.Automation@globalknowledge.com','Pallavi.Govindraj@globalknowledge.com','',null,v_mail_hdr,'CD Fee PO Report',v_file_name_full_po);
...
*/
--End changes made by SUBRAMANIANB to disable Oracle Auto-Receiving
exception
  when others then
    rollback;
