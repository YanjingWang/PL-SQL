DROP PROCEDURE GKDW.GK_CONF_EVENT_CHANGE_PROC;

CREATE OR REPLACE PROCEDURE GKDW.gk_conf_event_change_proc as

cursor c1 is
select replace(txfeeid,'Q6UJ9','EVCHG') attachment_id,
       ...
       gk_conf_reg_hdr_func(ed.event_id,'SCH') change_hdr,
       gk_conf_sched_change_func(initcap(c.first_name),ed.event_name,
                                 to_char(startdate,'Mon dd, yyyy')||' '||to_char(starttime,'hh:mi AM')||' '||tzabbreviation,
                                 to_char(enddate,'Mon dd, yyyy')||' '||to_char(endtime,'hh:mi AM')||' '||tzabbreviation
                                 ) change_body,
 ...
 where ea.enrollstatus = 'Confirmed'
   ...
   and (to_char(ec.starttime,'hh:mi:ss AM') <> ea.start_time
    or  to_char(ec.endtime,'hh:mi:ss AM') <> ea.end_time
    or  ec.tzabbreviation <> ea.timezone
    or  ec.startdate <> ea.start_date
    or  ec.enddate <> ea.end_date
       )
 order by 1;
...
for r1 in c1 loop
  ...
  select r1.evxevenrollid||'_GlobalKnowledgeEventChangeLetter_'||to_char(sysdate,'yyyy-mm-dd_hh24_mi_ss')||'.html'
    into v_file_name
    from dual;

  v_attach_desc := 'Global-Knowledge-Event-Change-Letter--Conf#-'||r1.evxevenrollid||'-('||r1.firstname||'-'||r1.lastname||')';
  v_curr_time := to_char(sysdate,'yyyy-mm-dd hh24:mi:ss');


/******* EVENT CHANGE EMAIL ********/
  v_msg_body := r1.conf_hdr;
  v_msg_body := v_msg_body||r1.change_hdr;
  v_msg_body := v_msg_body||r1.change_body;
  ...
  send_mail('ConfirmAdmin.NAm@globalknowledge.com','Confirmation.Test@globalknowledge.com','CC-Notification of Course Schedule Change',v_msg_body);
  send_mail('ConfirmAdmin.NAm@globalknowledge.com',r1.conf_email,'Notification of Course Schedule Change',v_msg_body);
  if r1.secondaryemail is not null then
    send_mail('ConfirmAdmin.NAm@globalknowledge.com',r1.secondaryemail,'Notification of Course Schedule Change',v_msg_body);
  end if;
  if r1.channel_flag = 'Y' then
    send_mail('ConfirmAdmin.NAm@globalknowledge.com',r1.channel_cc1_email,'Notification of Course Schedule Change - Conf# '||r1.evxevenrollid,v_msg_body);
    send_mail('ConfirmAdmin.NAm@globalknowledge.com',r1.channel_cc2_email,'Notification of Course Schedule Change - Conf# '||r1.evxevenrollid,v_msg_body);
  end if;

  update gk_conf_email_audit
     set start_time = r1.starttime,
         end_time = r1.endtime,
         timezone = r1.tzabbreviation,
         fac_change_date = sysdate,
         start_date = r1.startdate,
         end_date = r1.enddate
...
/******* OUTLOOK ENROLLMENT ********/
  if r1.cal_reminder = 'Y' and r1.channel_flag = 'N' then
    ...
    v_ical_event := ical_event_new(
      ...
      send_ical_email('ConfirmAdmin.NAm@globalknowledge.com',r1.conf_email,'Notification of Course Schedule Change ('||r1.evxevenrollid||')',v_msg_body,v_ical_event);
...
--    v_file := utl_file.fopen('/usr/tmp',v_file_name,'w',32767);
  ...
  utl_file.put_line(v_file,r1.change_hdr);
  utl_file.put_line(v_file,r1.change_body);
...
GRANT EXECUTE ON GKDW.GK_CONF_EVENT_CHANGE_PROC TO COGNOS_RO;
