DROP PROCEDURE GKDW.GK_CONF_EMAIL_PROC_2;

CREATE OR REPLACE PROCEDURE GKDW.gk_conf_email_proc_2 as
...
select replace(nvl(ec.txfeeid,ec.evxevenrollid),'Q6UJ9','CONFR') attachment_id,
       ...
       gk_conf_mygk_survey_func(ed.event_id,ec.contactid) event_survey,
 ...
 where ec.EVXEVENROLLID IN ('QGKID0A26Q67',
'QGKID0A26Q6I') --and
-- ec.enrollstatus = 'Confirmed'
  -- and ed.end_date >= trunc(sysdate)
   ...
   and not exists (select 1 from course_dim cd1 where cd.course_code = cd1.course_code and cd1.md_num = '32' and cd1.pl_num = '19' and ec.cust_country not in ('USA','CANADA'))
...
/******* Additional Information **********/
  ...
  send_mail('ConfirmAdmin.NAm@globalknowledge.com','Confirmation.Test@globalknowledge.com','CC-Important Course Access Details from Global Knowledge ('||r1.evxevenrollid||')',v_msg_body);
  send_mail('ConfirmAdmin.NAm@globalknowledge.com',r1.conf_email,'Important Course Access Details from Global Knowledge ('||r1.evxevenrollid||')',v_msg_body);
  if r1.secondaryemail is not null then
    send_mail('ConfirmAdmin.NAm@globalknowledge.com',r1.secondaryemail,'Important Course Access Details from Global Knowledge ('||r1.evxevenrollid||')',v_msg_body);
  end if;
  if r1.channel_flag = 'Y' then
    send_mail('ConfirmAdmin.NAm@globalknowledge.com',r1.channel_cc1_email,'Global Knowledge Order Confirmation ('||r1.evxevenrollid||'-'||r1.firstname||' '||r1.lastname||'-'||r1.course_code||')',v_msg_body);
    send_mail('ConfirmAdmin.NAm@globalknowledge.com',r1.channel_cc2_email,'Global Knowledge Order Confirmation ('||r1.evxevenrollid||'-'||r1.firstname||' '||r1.lastname||'-'||r1.course_code||')',v_msg_body);
  end if;
...
/******* OUTLOOK ENROLLMENT ********/
  if r1.cal_reminder = 'Y' and r1.channel_flag = 'N' then
    ...
    send_ical_email('ConfirmAdmin.NAm@globalknowledge.com',r1.conf_email,'Global Knowledge Training Event ('||r1.evxevenrollid||')',v_msg_body,v_ical_event);
...
exception
  when others then
    rollback;
    send_mail('DW.Automation@globalknowledge.com','DW.Automation@globalknowledge.com','GK_CONF_EMAIL_PROC Failed',SQLERRM);
