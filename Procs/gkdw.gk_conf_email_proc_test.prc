DROP PROCEDURE GKDW.GK_CONF_EMAIL_PROC_TEST;

CREATE OR REPLACE PROCEDURE GKDW.gk_conf_email_proc_test as
...
select distinct replace(nvl(ec.txfeeid,ec.evxevenrollid),'Q6UJ9','CONFR') attachment_id,
 ...
 where ec.evxevenrollid  in ('QGKID0B0OTII')--'Q6UJ90A54NKN','QGKID0AG4KQZ') --, 'Q6UJ90AL309H')  
...
exception 
  when others then
    send_mail('DW.Automation@globalknowledge.com','DW.Automation@globalknowledge.com','GK_CONF_EMAIL_PROC error in evxevenrollid: '||r1.evxevenrollid,SQLERRM);  
end;
