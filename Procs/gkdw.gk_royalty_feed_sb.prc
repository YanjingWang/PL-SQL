DROP PROCEDURE GKDW.GK_ROYALTY_FEED_SB;

CREATE OR REPLACE PROCEDURE GKDW.gk_royalty_feed_SB(p_week varchar2 default null,p_test varchar2 default 'N') as
cursor c0 is
...
cursor c1(vcode varchar2,vweek varchar2) is
  select ops_country,org_id,inv_org_id,curr_code,oracle_vendor_id,oracle_vendor_site
   ...
   where vendor_code = vcode
     and po_active = 'Y'
     and ((dim_week = vweek and upper(po_create_meth) = 'DELIVERY' and to_char(sysdate-1,'d') = '5')
      or  (upper(po_create_meth) = 'ORDER' and book_date = trunc(sysdate)-2))
...
if p_week is not null then
  ...
  select td.dim_year||'-'||lpad(td.dim_week,2,'0') into v_curr_week from time_dim td where trunc(sysdate-2) = dim_date;
end if;
