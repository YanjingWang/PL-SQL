DROP PROCEDURE GKDW.GK_TERILLIAN_INST_LOAD_NEW;

CREATE OR REPLACE PROCEDURE GKDW.gk_terillian_inst_load_new as
...
cursor c1 is
    ...
    or    (GTE.COURSE_CODE in ( '9995L'/*,  '9995V'*/) and trunc(gte.end_date) > trunc(sysdate) - 1) -- SR ticket# 6572
...
begin
    ...
    select 'instructors.txt'
    into v_audit_file
    from dual;

    v_file := utl_file.fopen('/mnt/shared_ww/lods',v_audit_file,'w');
