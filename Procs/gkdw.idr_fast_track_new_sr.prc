DROP PROCEDURE GKDW.IDR_FAST_TRACK_NEW_SR;

CREATE OR REPLACE PROCEDURE GKDW.idr_fast_track_new_sr (
   ...
   p_fast_tract            OUT SYS_REFCURSOR)
AS
   v_fast_tract   SYS_REFCURSOR;
...
BEGIN
   ...
   THEN
      OPEN v_fast_tract FOR
           SELECT SUBSTR (q.course_code, 1, LENGTH (q.course_code) - 1)
                     course_code,
                  SUBSTR (q.course_code, -1) modality,
                  TO_CHAR (dim_date, 'yyyy-mm-dd') request_date,
                  TO_CHAR (
   ...
   ELSE
      OPEN v_fast_tract FOR
           SELECT SUBSTR (cd.course_code, 1, LENGTH (cd.course_code) - 1),
                  SUBSTR (cd.course_code, -1),
                  TO_CHAR (td2.dim_date, 'yyyy-mm-dd'),
                  TO_CHAR (
