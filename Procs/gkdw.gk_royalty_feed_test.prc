DROP PROCEDURE GKDW.GK_ROYALTY_FEED_TEST;

CREATE OR REPLACE PROCEDURE GKDW.gk_royalty_feed_TEST(p_week varchar2 default null,p_test varchar2 default 'N') as
cursor c0 is
...
--                Attachments  => SendMailJPkg.ATTACHMENTS_LIST(v_file_full_po));
  ...
  if r0.event_audit = 'Y' --and to_char(sysdate,'d') = '5' 
  then
...
--if to_char(sysdate,'d') = '5' then
  ...
  if p_test = 'Y' then
    ...
    v_error:= SendMailJPkg.SendMail(
              SMTPServerName => 'corpmail.globalknowledge.com',
              Sender    => 'DW.Automation@globalknowledge.com',
              Recipient => 'Marion.Kennelly@globalknowledge.com', --'erica.loring@globalknowledge.com',
              CcRecipient => '',
              BccRecipient => 'DW.Automation@globalknowledge.com',
              Subject   => v_mail_hdr,
              Body => 'GK Weekly Audit Reports',
