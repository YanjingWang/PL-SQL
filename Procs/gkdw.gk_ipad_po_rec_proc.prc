DROP PROCEDURE GKDW.GK_IPAD_PO_REC_PROC;

CREATE OR REPLACE PROCEDURE GKDW.gk_ipad_po_rec_proc as

cursor c4 is
select distinct h.segment1,h.po_header_id,l.po_line_id,h.vendor_id,h.vendor_site_id,
       d.deliver_to_person_id,trunc(sysdate) ship_date,
       d.destination_organization_id,l.category_id,
       l.line_num,l.item_description,l.quantity,
       trunc(sysdate) deliv_date,
  ...
  from po_headers_all@r12prd h
       ...
       inner join gk_promo_fulfilled_orders f on h.segment1 = f.po_num and l.line_num = f.po_line_num and f.tracking_num is not null
 where d.quantity_delivered < l.quantity
...
for r4 in c4 loop
    ...
    insert into rcv_transactions_interface@r12prd
      ...
      select v_rcv_transaction,v_rcv_header,v_rcv_interface,
             sysdate,0,sysdate,0,'RECEIVE',trunc(sysdate),
             'PENDING','BATCH','PENDING',r4.qty_deliv,
