DROP PROCEDURE GKDW.GK_IBM_RSS_PROC_SB;

CREATE OR REPLACE PROCEDURE GKDW.gk_ibm_rss_proc_sb as



--  =======================================================================
--  PROGRAM NAME : gk_ibm_rss_proc
--  PURPOSE      : Process the IBM RSS FEED
--  Note         :
--
--  UPDATE HISTORY
...
--  -----------  -----    ---------------------    -------------
--  23-Jul-2007  1.0                               New
--  19-Feb-2020  1.1       Smaranika Baral         GK schedule not on Skills Gateway. Also wrong Timezone is reflecting  in Skills Gateway
--  19-May-2020  1.2      Smaranika Baral         Ticket #24888 : IBM RSS FEED: Wrong DurationUnit on Skills Gateway
-- =======================================================================
...
select chr(9)||'<item>'||chr(10)||
       ...
       chr(9)||chr(9)||'<course:TimeZone>'||case when cd.md_num in ('32','44') then null when trunc(et.offsetfromutc) <> et.offsetfromutc then to_char(trunc(et.offsetfromutc),'FM00')||':30' else to_char(trunc(et.offsetfromutc),'FM00')||':00' end||'</course:TimeZone>'||chr(10)|| /*Change 1.1 Added formatting 'FM00'*/
...
select chr(9)||'<item>'||chr(10)||
       ...
       chr(9)||chr(9)||'<course:DurationUnits>D</course:DurationUnits>'||chr(10)||  /* Ticket #24888 : Changed the duration unit to D*/
       chr(9)||chr(9)||'<course:IRLPSupport>0</course:IRLPSupport>'||chr(10)||
       chr(9)||chr(9)||'<course:VirtualWorkstationRequired>0</course:VirtualWorkstationRequired>'||chr(10)||
...
select chr(9)||'<item>'||chr(10)||
       ...
       chr(9)||chr(9)||'<course:DurationUnits>'||case when p.duration_days is not null then 'D'
                                                      else tx.duration_unit
                                                 end ||'</course:DurationUnits>'||chr(10)||  /* Ticket #24888 : Changed the logic for duration unit */
       chr(9)||chr(9)||'<course:IRLPSupport>0</course:IRLPSupport>'||chr(10)||
       chr(9)||chr(9)||'<course:VirtualWorkstationRequired>0</course:VirtualWorkstationRequired>'||chr(10)||
...
select event_id,course_code,course_name,ibm_ww_course_code,start_date,end_date,replace(city,chr(38)||'atilde;','a') city,country,state,delivery_method,duration_length,class_type,
 ...
 select p.event_id,p.course_code,tx.title course_name,p.ibm_ww_course_code,p.start_date,p.end_date,replace(p.city,chr(38)||'atilde;','a') city,p.country,p.state,p.delivery_method,to_number(p.duration_days) duration_length,p.class_type,
        ...
        p.course_url,p.class_language,'D' as duration_unit,  /* Ticket #24888 : Changed the logic for duration unit */
 ...
 select p.iso_ctry||substr(p.event_id,3),p.course_code,r.course_name,p.ibm_ww_course_code,p.start_date,p.end_date,replace(p.city,chr(38)||'atilde;','a') city,p.iso_ctry country,p.state,p.delivery_method,
       ...
       replace(replace(p.start_time,'00:00','00'),'30:00','30') start_time,
       replace(replace(p.end_time,'00:00','00'),'30:00','30') end_time,
       p.course_url,p.class_language,(case when p.duration_days is not null then 'D' 
                                          else tx.duration_unit end) duration_unit,  /* Ticket #24888 : Changed the logic for duration unit */
       null ext_partner,
       case when trim(p.gtr) is null then '0'
            when trim(p.gtr) = 'F' then '0'
            when trim(p.gtr) = 'T' then '1'
 ...
 select event_id,course_code,course_name,ibm_course_code,start_date,end_date,city,isoctry,state_abbrv,modality,duration_days,classtype,
        ...
        to_char(course_url),class_language,'D' as duration_unit, /* Ticket #24888 : Changed the logic for duration unit */
        case when ext_partner in ('Global Knowledge S.A. de C.V.','Global Knowledge Asia PTE Ltd.') then null else ext_partner end ext_partner,gtr_flag
...
select chr(9)||'<item>'||chr(10)||
       ...
       chr(9)||chr(9)||'<course:DurationUnits>D</course:DurationUnits>'||chr(10)|| /* Ticket #24888 : Changed the logic for duration unit */
       chr(9)||chr(9)||'<course:IRLPSupport>0</course:IRLPSupport>'||chr(10)||
       chr(9)||chr(9)||'<course:VirtualWorkstationRequired>0</course:VirtualWorkstationRequired>'||chr(10)||
       chr(9)||chr(9)||'<course:ClassType>'||initcap(replace(nvl(class_type,'Public'),chr(13),''))||'</course:ClassType>'||chr(10)||
       chr(9)||chr(9)||'<course:TimeZone>'||replace(timezone,'UTC','')||'</course:TimeZone>'||chr(10)||
...
insert into sb_test values ('STEP5');
...
commit;
insert into sb_test values ('STEP6');
...
insert into sb_test values ('STEP7');
...
insert into sb_test values ('STEP8');
...
exception
  when others then
    rollback;
    send_mail('DW.Automation@globalknowledge.com','smaranika.baral@globalknowledge.com','GK_IBM_RSS_PROC Failed',SQLERRM);
