DROP PROCEDURE GKDW.GK_PPLCERT_EXAMS_PROC;

CREATE OR REPLACE PROCEDURE GKDW.gk_pplCert_exams_proc is
...
select 'PeopleCert_exam_'||to_char(sysdate,'yyyymmddhh24miss')||'.xls',
       '/usr/tmp/PeopleCert_exam_'||to_char(sysdate,'yyyymmddhh24miss')||'.xls',
       'PeopleCert_exam_detail_'||to_char(sysdate,'yyyymmddhh24miss')||'.xls',
       '/usr/tmp/PeopleCert_exam_detail_'||to_char(sysdate,'yyyymmddhh24miss')||'.xls'
...
v_error:= SendMailJPkg.SendMail(
          ...
          CcRecipient => 'Jamie.clark@globalknowledge.com',
          BccRecipient => '',
          Subject   => 'PEOPLECERT EXAM PO FOR WEEK OF '||v_po_week,
          Body => 'Open Attached File',
          ErrorMessage => v_error_msg,
...
v_error:= SendMailJPkg.SendMail(
          SMTPServerName => 'corpmail.globalknowledge.com',
          Sender    => 'cait.bauer@globalknowledge.com',
          Recipient => 'exams@peoplecert.org',
          CcRecipient => '',
          BccRecipient => '',
          Subject   => 'PEOPLECERT EXAM PO FOR WEEK OF '||v_po_week,
          Body => 'Open Attached File',
