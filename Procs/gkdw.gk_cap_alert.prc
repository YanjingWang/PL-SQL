DROP PROCEDURE GKDW.GK_CAP_ALERT;

CREATE OR REPLACE PROCEDURE GKDW.gk_cap_alert as
...
utl_file.fclose(c_file);
  ...
  v_error:= SendMailJPkg.SendMail(
                ...
                Recipient => 'capalerts.nam@globalknowledge.com',
                CcRecipient => '',
                BccRecipient => '',
                Subject   => v_mail_hdr,
                Body => 'Daily Cap Increase',
                ErrorMessage => v_error_msg,
                Attachments  => SendMailJPkg.ATTACHMENTS_LIST(cap_file_full));
