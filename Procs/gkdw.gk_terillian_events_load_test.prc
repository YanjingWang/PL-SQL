DROP PROCEDURE GKDW.GK_TERILLIAN_EVENTS_LOAD_TEST;

CREATE OR REPLACE PROCEDURE GKDW.gk_terillian_events_load_test as
   --=======================================================================
...
select nvl( GTE.TERILLIAN_COURSE_CODE, GTE.COURSE_CODE) course_code,class_id,replace(event_name,',') event_name,status,location_id, 
 ...
 where trunc(gte.end_date) between trunc(sysdate)-1 and trunc(sysdate)+120 or course_code = '9995V'
union
select nvl( GTE.TERILLIAN_COURSE_CODE, GTE.COURSE_CODE) course_code,class_id,replace(event_name,',') event_name,status,location_id, 
       company_id,maxseats,minseat,start_time,end_time,connected_c,connected_v_to_c, 
       start_date,end_date,tzgenericname,country
...
select 'Events_test.txt','Sessions_test.txt'
