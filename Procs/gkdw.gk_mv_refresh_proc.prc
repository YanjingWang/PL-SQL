DROP PROCEDURE GKDW.GK_MV_REFRESH_PROC;

CREATE OR REPLACE PROCEDURE GKDW.gk_mv_refresh_proc
AS
   --=======================================================================
   ...
   --                                          daily refresh
   -- 1.4       06/03/2020   Smaranika Baral   Tkt #28430 : Added RMSDW.rms_course_times_mv' 
   --                                          for daily refresh
   --========================================================================
...
BEGIN
   ...
   v_count := 470;
   DBMS_SNAPSHOT.REFRESH ('GKDW.MASTER_ENT_BOOKINGS_MV_$0');
   DBMS_OUTPUT.put_line ('MASTER_ENT_BOOKINGS_MV_$0 complete: ' || TO_CHAR (SYSDATE, 'hh24:mi:ss'));
   v_count := 480;
   DBMS_SNAPSHOT.REFRESH ('GKDW.MASTER_ENT_OE_BOOKING_MV_$0');
   DBMS_OUTPUT.put_line ('MASTER_ENT_OE_BOOKING_MV_$0 complete: ' || TO_CHAR (SYSDATE, 'hh24:mi:ss'));

 -- Tkt #28430 : Changes begin

   --   v_count := 470;
--   DBMS_SNAPSHOT.REFRESH ('RMSDW.rms_course_times_mv');
 --  DBMS_OUTPUT.put_line ('rms_course_times_mv complete: ' || TO_CHAR (SYSDATE, 'hh24:mi:ss'));

 -- Tkt #28430 : Changes end
--   v_count := 470;
...
--   DBMS_OUTPUT.put_line ('master_ent_oe_booking_mv_test2 complete: ' || TO_CHAR (SYSDATE, 'hh24:mi:ss'));
 ...
 --  send_mail ('DW.Automation@globalknowledge.com','katie.santos@globalknowledge.com','GK_MV_REFRESH_PROC completed Successfully.','Warehouse MV refresh is now complete');
   ...
   send_mail ('DW.Automation@globalknowledge.com','Alan.frelich@globalknowledge.com','4:00 AM Refresh-GK_MV_REFRESH_PROC completed Successfully .','Warehouse MV refresh is now complete');
   send_mail ('DW.Automation@globalknowledge.com','Smaranika.baral@globalknowledge.com','4:00 AM Refresh-GK_MV_REFRESH_PROC completed Successfully.','Warehouse MV refresh is now complete');
  send_mail ('DW.Automation@globalknowledge.com','Bala.Subramanian@globalknowledge.com','4:00 AM Refresh-GK_MV_REFRESH_PROC completed Successfully.','Warehouse MV refresh is now complete');
EXCEPTION
   WHEN OTHERS
   THEN
      v_msg_body :=
      ...
      send_mail ('DW.Automation@globalknowledge.com',
                 ...
                 '4:00 AM Refresh-GK_MV_REFRESH_PROC FAILED',
                v_msg_body);
       send_mail ('DW.Automation@globalknowledge.com',
...
GRANT EXECUTE ON GKDW.GK_MV_REFRESH_PROC TO DWHREAD;
