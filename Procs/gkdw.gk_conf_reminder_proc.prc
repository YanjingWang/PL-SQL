DROP PROCEDURE GKDW.GK_CONF_REMINDER_PROC;

CREATE OR REPLACE PROCEDURE GKDW.gk_conf_reminder_proc as
...
/******* Additional Information **********/
  ...
  send_mail('ConfirmAdmin.NAm@globalknowledge.com','Confirmation.Test@globalknowledge.com','CC-Your Global Knowledge course is next week',v_msg_body);
  send_mail('ConfirmAdmin.NAm@globalknowledge.com',r1.conf_email,'Your Global Knowledge course is next week',v_msg_body);
  if r1.secondaryemail is not null then
    send_mail('ConfirmAdmin.NAm@globalknowledge.com',r1.secondaryemail,'Your Global Knowledge course is next week',v_msg_body);
  end if;
...
--  v_file := utl_file.fopen('/usr/tmp',v_file_name,'w',32767);
  ...
  utl_file.put_line(v_file,r1.rem_hdr);
  utl_file.put_line(v_file,r1.rem_body);
...
exception
  when others then
    rollback;
    send_mail('DW.Automation@globalknowledge.com','DW.Automation@globalknowledge.com','GK_CONF_REMINDER_PROC Failed',SQLERRM);
...
GRANT EXECUTE ON GKDW.GK_CONF_REMINDER_PROC TO COGNOS_RO;
