DROP PROCEDURE GKDW.GK_MYGK_INSTR_EMAIL_EVENT_PROC;

CREATE OR REPLACE PROCEDURE GKDW.gk_mygk_instr_email_event_proc(p_event_id varchar2) as
...
select distinct ie.contactid,ie.firstname||' '||ie.lastname inst_name,ie.email inst_email
  from event_dim ed
       ...
       inner join instructor_event_v ie on ed.event_id = ie.evxeventid and ie.feecode = 'INS'
...
select ed.event_id,ed.facility_region_metro,ed.location_name,ed.facility_code,
  ...
  from event_dim ed
       ...
       inner join instructor_event_v ie on ed.event_id = ie.evxeventid and feecode = 'INS'
       inner join gk_mygk_course_profile_v p on cd.course_id = p.course_id
...
for r0 in c0 loop
  ...
  v_msg_body := v_msg_body||'<tr><th align=left>Instructor:</th><td align=left>'||v_inst||'</td></tr>';
  ...
  send_mail('ConfirmAdmin.NAM@globalknowledge.com',r0.inst_email,'MyGK Access URLs',v_msg_body);
end loop;
...
if v_mail_flag = 'Y' then
  send_mail_attach('DW.Automation@globalknowledge.com','instructor.helpdesk.us@globalknowledge.com','greg.rogers@globalknowledge.com','',v_file_name,'Open Excel Attachment.',v_file_name_full);
  send_mail_attach('DW.Automation@globalknowledge.com','scott.brown@globalknowledge.com',null,null,v_file_name,'Open Excel Attachment.',v_file_name_full);
