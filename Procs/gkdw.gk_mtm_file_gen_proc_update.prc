DROP PROCEDURE GKDW.GK_MTM_FILE_GEN_PROC_UPDATE;

CREATE OR REPLACE PROCEDURE GKDW.gk_mtm_file_gen_proc_update(p_date date default null,p_test varchar2 default 'N') as
...
cursor c2(t_date date,v_ops_country varchar2) is
  select distinct chr(9)||'<course xid="'||xml_course_id||'" name="'||
         case
         ...
         case when short_name is not null then '" mtmname="'||short_name else '' end  
		 ||'" ol="'||
...
if p_test = 'Y' then
  v_error:= SendMailJPkg.SendMail(
            ...
            CcRecipient => 'JAMAAL.JONES@globalknowledge.com',
