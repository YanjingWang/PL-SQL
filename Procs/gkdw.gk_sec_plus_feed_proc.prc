DROP PROCEDURE GKDW.GK_SEC_PLUS_FEED_PROC;

CREATE OR REPLACE PROCEDURE GKDW.gk_sec_plus_feed_proc as
/* Coding string changes for Spel W courses */
cursor c1 is
select enroll_id,event_id,start_date,cust_id,first_name,last_name,email,course_code,
       ...
       case when upper(ops_country) in ('CA','CAN','CANADA') then '220' 
            when ch_num = '20' and upper(country) in ('CA','CAN','CANADA') and substr(course_code,-1,1) != 'W' then '220' -- SR 09/19/2017: update country and added and substr(course_code,-1,1) != 'W' 
            when upper(country) in ('CA','CAN','CANADA') and substr(course_code,-1,1) = 'W' then '220'
            else '210' 
       end le_num,
       case when substr(course_code,-1,1) = 'W' then '000' else '130' end fe_num,
       case when substr(course_code,-1,1) = 'W' then '41315' else '62405' end acct_num,
       ch_num,
       case when substr(course_code,-1,1) = 'W' then '44' else md_num end md_num,
       pl_num,act_num,
       case when substr(course_code,-1,1) = 'W' then '000' else '200' end cc_num,
...
if c1%found then
  ...
  for r1 in c1 loop
    ...
    utl_file.put_line(v_file,r1.v_line1||','||v_curr_po||','||r1.v_line2);
...
--  v_ncftp_cmd := '/home/oracle/ncftp-3.2.5/bin/ncftpput -P 2121 -u globalknowledge -p t8swEYAmeguc 192.69.78.7 / '||v_file_name_full;
  ...
  v_ncftp_cmd := '/home/oracle/ncftp-3.2.5/bin/ncftpput -P 2121 -u globalknowledge -p t8swEYAmeguc 192.69.78.14 / '||v_file_name_full; -- SR 03/13/2018
...
exception
  when others then
    send_mail('DW.Automation@globalknowledge.com','DW.Automation@globalknowledge.com','GK_SEC_PLUS_FEED_PROC',SQLERRM);
