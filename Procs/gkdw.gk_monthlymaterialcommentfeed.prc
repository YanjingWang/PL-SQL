DROP PROCEDURE GKDW.GK_MONTHLYMATERIALCOMMENTFEED;

CREATE OR REPLACE PROCEDURE GKDW.gk_MonthlyMaterialCommentFeed(p_month varchar2) as
...
select   chr(9)||chr(9)|| '<Year4Digit>'|| M.YEAR4DIGIT || '</Year4Digit>' || chr(13) || chr(10) ||
    ...
    chr(9)||chr(9)|| '<MaterialComments>' v_xml_line,
    '</MaterialComments>' || chr(13) || chr(10) ||
 ...
 from 
   (     select * from  gk_mtm_matcomments m
)m
...
SELECT  distinct 
   chr(9)||chr(9)||  M.matcomments    matcomments
from 
   (            
         select distinct  
...
, CourseCodeIBMSec
 , DerivativeWork
  , "Modality", 
   instcomments matcomments
    from gk_mtm_matcomments    
)m
...
if c1%found then
  ...
  select 'ProviderMonthlyCommentFeed.xml','/usr/tmp/ProviderMonthlyCommentFeed.xml'
  ...
  select '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'|| chr(13)||chr(10)||
         '<Feed>'|| chr(13)||chr(10)||
         chr(9)|| '<FeedName>ProviderMonthlyMaterialCommentFeedV3</FeedName>'|| chr(13)||chr(10)||
...
--    DBMS_OUTPUT.PUT_LINE(r1.ibm_coursecode);
    ...
    loop
        ...
        if r2.matcomments is not null then 
        DBMS_OUTPUT.PUT_LINE(r2.matcomments);
            utl_file.put_line(xml_file, r2.matcomments) ;
        end if;
