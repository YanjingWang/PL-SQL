DROP PROCEDURE GKDW.GK_CONF_CANC_SUB_PROC;

CREATE OR REPLACE PROCEDURE GKDW.gk_conf_canc_sub_proc as
...
select replace(nvl(ec.txfeeid,ec.evxevenrollid),'Q6UJ9','CANCL') attachment_id,
       ...
       gk_conf_reg_hdr_func(ed.event_id,'SUB') canc_hdr,
       gk_conf_canc_func('SUB',ed.event_id,initcap(ec.firstname),ea.channel_flag,trim(ec.enrollstatusdesc),ec.eventname) canc_body,
 ...
 where ec.enrollstatus = 'Cancelled'
   ...
   and ec.enrollstatusdesc = 'CANCELLATION - STUDENT SUBSTITUTION';
...
/******* CANCELLATION EMAIL ***********/
  ...
  send_mail('ConfirmAdmin.NAm@globalknowledge.com','Confirmation.Test@globalknowledge.com','CC-Course substitution confirmation ('||r3.evxevenrollid||')',v_msg_body);
  send_mail('ConfirmAdmin.NAm@globalknowledge.com',r3.conf_email,'Course substitution confirmation ('||r3.evxevenrollid||')',v_msg_body);
  if r3.secondaryemail is not null then
    send_mail('ConfirmAdmin.NAm@globalknowledge.com',r3.secondaryemail,'Course substitution confirmation ('||r3.evxevenrollid||')',v_msg_body);
  end if;
  if r3.channel_flag = 'Y' then
    send_mail('ConfirmAdmin.NAm@globalknowledge.com',r3.channel_cc1_email,'Course substitution confirmation ('||r3.evxevenrollid||')',v_msg_body);
    send_mail('ConfirmAdmin.NAm@globalknowledge.com',r3.channel_cc2_email,'Course substitution confirmation ('||r3.evxevenrollid||')',v_msg_body);
  end if;
...
/******* OUTLOOK CANCELLATION ********/
  if r3.cal_reminder = 'Y' and r3.channel_flag = 'N' then
    v_ical_event := ical_event_new(
      ...
      send_ical_email('ConfirmAdmin.NAm@globalknowledge.com',r3.conf_email,'Course substitution confirmation ('||r3.evxevenrollid||')',v_msg_body,v_ical_event);
  end if;
...
exception
  when others then
    rollback;
    send_mail('DW.Automation@globalknowledge.com','DW.Automation@globalknowledge.com','GK_CONF_CANC_SUB_PROC Failed',SQLERRM);

end;
/


GRANT EXECUTE ON GKDW.GK_CONF_CANC_SUB_PROC TO COGNOS_RO;
