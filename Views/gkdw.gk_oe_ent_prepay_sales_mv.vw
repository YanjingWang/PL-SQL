DROP VIEW GKDW.GK_OE_ENT_PREPAY_SALES_MV;

/* Formatted on 29/01/2021 11:31:56 (QP5 v5.115.810.9015) */
CREATE OR REPLACE FORCE VIEW GKDW.GK_OE_ENT_PREPAY_SALES_MV
(
   "Transaction ID",
   "Cust Account",
   "Cust Country",
...
AS
   SELECT   DISTINCT
     ...
     FROM                                                            order_fact f
               ...
               LEFT OUTER JOIN
                  gk_territory gtt
               ON gtt.territory_id = rt.ob_comm_type
            LEFT OUTER JOIN
               userinfo@slx ui9
