DROP VIEW GKDW.GK_PRODUCT_FAC_EXT_COST_V;

/* Formatted on 29/01/2021 11:29:37 (QP5 v5.115.810.9015) */
CREATE OR REPLACE FORCE VIEW GKDW.GK_PRODUCT_FAC_EXT_COST_V
(
   COST_TYPE,
   EVENT_ID,
   EVENT_DESC,
...
AS
     SELECT   'External Facility Cost' cost_type,
              ed.event_id,
                 cd.course_code
              || '-'
              || facility_region_metro
       ...
       FROM                     event_dim ed
                       ...
                       INNER JOIN
                          gk_ext_fac_event_cost_v fe
                       ON ed.event_id = fe.event_id
                    LEFT OUTER JOIN
