DROP VIEW GKDW.GK_COURSE_FILL_RATE_V;

/* Formatted on 29/01/2021 11:40:01 (QP5 v5.115.810.9015) */
CREATE OR REPLACE FORCE VIEW GKDW.GK_COURSE_FILL_RATE_V
(
   OPS_COUNTRY,
   METRO,
   COURSE_ID,
...
AS
     SELECT   ed.ops_country,
              ...
              CASE
                 WHEN ab.avg_book_amt = 0 THEN 0
                 ELSE ROUND ( (AVG (total_cost) * 1.8) / ab.avg_book_amt)
              END
                 avg_fill,
              CASE
                 WHEN ab.avg_book_amt = 0 THEN 0
                 ELSE ROUND ( (MAX (total_cost) * 1.8) / ab.avg_book_amt)
              END
                 max_fill
