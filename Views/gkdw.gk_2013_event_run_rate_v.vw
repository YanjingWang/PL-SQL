DROP VIEW GKDW.GK_2013_EVENT_RUN_RATE_V;

/* Formatted on 29/01/2021 11:21:40 (QP5 v5.115.810.9015) */
CREATE OR REPLACE FORCE VIEW GKDW.GK_2013_EVENT_RUN_RATE_V
(
   COURSE_CODE,
   METRO,
   EVENT_CNT,
...
AS
     SELECT   cd.course_code,
      ...
      WHERE       td.dim_year >= TO_NUMBER (TO_CHAR (SYSDATE, 'yyyy')) - 2
              AND cd.ch_num = '10'
              AND cd.md_num = '10'
   GROUP BY   cd.course_code, ed.facility_region_metro;
