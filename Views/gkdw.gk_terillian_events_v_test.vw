DROP VIEW GKDW.GK_TERILLIAN_EVENTS_V_TEST;

/* Formatted on 29/01/2021 11:25:22 (QP5 v5.115.810.9015) */
CREATE OR REPLACE FORCE VIEW GKDW.GK_TERILLIAN_EVENTS_V_TEST
(
   COURSE_ID,
   COURSE_CODE,
   CLASS_ID,
...
AS
   SELECT   DISTINCT
    ...
    WHERE   lc."lab" = 293 AND ed.course_code <> '9995V'
            AND SUBSTR (cd.course_code, -1, 1) IN
                     ('C', 'L', 'N', 'V', 'B', 'Z')
...
--          LEFT JOIN "schedule"@rms_prod s ON s."slx_id" = ed.EVENT_ID
--          LEFT JOIN "location_func"@rms_prod lf
--             ON s."location_func" = lf."id"
--          LEFT JOIN "SLX_TIMEZONE"@rms_prod tz
--             ON lf."timezone" = tz."OFF_SET"
--          INNER JOIN course_dim cd
--             ON ed.course_id = cd.course_id AND ed.ops_country = cd.country
--    WHERE     (SUBSTR (course_code, 1, 4) IN ('3326',
--                                              '3405',
--                                              '3410',
--                                              '3411',
--                                              '9995'))
...
--          cd.md_num
--     FROM event_dim ed
--          INNER JOIN gk_mygk_course_profile_v p
--             ON ed.course_code = p.course_code
--          INNER JOIN evxevent ee ON ed.event_id = ee.evxeventid
--          LEFT OUTER JOIN "schedule"@rms_prod s ON s."slx_id" = ed.event_id
--          LEFT OUTER JOIN "location_func"@rms_prod lf
--             ON s."location_func" = lf."id"
--          INNER JOIN "SLX_TIMEZONE"@rms_prod tz
--             ON lf."timezone" = tz."OFF_SET"
--          INNER JOIN course_dim cd
--             ON ed.course_id = cd.course_id AND ed.ops_country = cd.country
...
--          cd.md_num
--     FROM event_dim ed2
--          INNER JOIN event_dim ed ON ed2.connected_v_to_c = ed.event_id
--          INNER JOIN gk_mygk_course_profile_v p
--             ON ed.course_code = p.course_code
--          INNER JOIN evxevent ee ON ed.event_id = ee.evxeventid
--          LEFT OUTER JOIN "schedule"@rms_prod s ON s."slx_id" = ed.event_id
