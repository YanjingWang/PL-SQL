DROP VIEW GKDW.GK_PRODUCT_TRAVEL_COST_V;

/* Formatted on 29/01/2021 11:28:57 (QP5 v5.115.810.9015) */
CREATE OR REPLACE FORCE VIEW GKDW.GK_PRODUCT_TRAVEL_COST_V
(
   COST_TYPE,
   EVENT_ID,
   EVENT_DESC,
...
AS
     SELECT   trav_grp cost_type,
              ...
              cd.vendor_code,
              SUM (NVL (trav_cost, 0)) inst_amt
       FROM                     inst_travel_cost_v@gkprod tc
                             INNER JOIN
                                event_dim ed
                             ON tc.evxeventid = ed.event_id
                          INNER JOIN
                             time_dim td
