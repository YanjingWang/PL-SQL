DROP VIEW GKDW.GK_SCHED_1_AGO_V;

/* Formatted on 29/01/2021 11:27:04 (QP5 v5.115.810.9015) */
CREATE OR REPLACE FORCE VIEW GKDW.GK_SCHED_1_AGO_V
(
   DIM_YEAR,
   COUNTRY,
   COURSE_CH,
...
AS
     SELECT   TO_CHAR (SYSDATE, 'YYYY') - 1 dim_year,
              country,
              course_ch,
              course_mod,
              course_pl,
       ...
       FROM   (  SELECT   cd.country,
                  ...
                  WHERE       td.dim_year = TO_CHAR (SYSDATE, 'YYYY') - 1
                          AND ch_num = '10'
                          AND md_num IN ('10', '20')
               ...
               GROUP BY   cd.country,
                          ...
                          cd.short_name,
                          cd.duration_days)
   GROUP BY   TO_CHAR (SYSDATE, 'YYYY') - 1,
              country,
              course_ch,
              course_mod,
              course_pl,
