DROP VIEW GKDW.GK_CC_DETAILED_V;

/* Formatted on 29/01/2021 11:41:42 (QP5 v5.115.810.9015) */
CREATE OR REPLACE FORCE VIEW GKDW.GK_CC_DETAILED_V
(
   QG_CCLOGGINGID,
   ORDERID,
   SLX_CONTACT_ID,
...
AS
   SELECT   qg_ccloggingid,
     ...
     FROM            (SELECT   qg_ccloggingid,
                       ...
                       WHERE   (TRUNC (c.createdate) >= TRUNC (SYSDATE) - 45 --TO_DATE ('04/01/2018', 'mm/dd/yyyy')
                                OR TRUNC (oh.createdate) >=
                                     TRUNC (SYSDATE) - 45 --TO_DATE ('04/01/2018', 'mm/dd/yyyy')
                                                         )
   ...
   SELECT   qg_ccloggingid,
               ...
               (SELECT   qg_ccloggingid,
                  ...
                  FROM      slxdw.qg_cclogging c
                         ...
                         ON c.authcode = oh.CC_AUTHORIZATIONNUMBER
                            AND oh.cc_number = c.creditcardnumber
                 WHERE   (TRUNC (c.createdate) >= TRUNC (SYSDATE) - 45 --TO_DATE ('04/01/2018', 'mm/dd/yyyy')
                          OR TRUNC (oh.createdate) >= TRUNC (SYSDATE) - 45 --TO_DATE ('04/01/2018', 'mm/dd/yyyy')
                                                                          )
    ...
    WHERE   bp.method = 'Credit Card'
            AND TRUNC (oh.createdate) >= TRUNC (SYSDATE) - 45 --TO_DATE ('04/01/2018', 'mm/dd/yyyy');
